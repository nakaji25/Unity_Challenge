...
public class GameManager : MonoBehaviour
{
    ...
    public GameObject[] buttomLanp = new GameObject[3]; //ボタン：金庫
    ...
    private bool doseHaveHammer; //トンカチを持っているか？
    ...
    void Start()
    {
        ...
        doseHaveHammer = false; //トンカチは「持っていない」
    ...
    void ChangeButtonColor(int buttonNo)
    {
        ...
        //ボタンの画像を変更
        buttomLanp[buttonNo].GetComponent<Image>().sprite = buttonPicture[buttonColor[buttonNo]];

        //ボタンの色順をチェック
        if ((buttonColor[0] == COLOR_BLUE) && (buttonColor[1] == COLOR_WHITE) && (buttonColor[2] == COLOR_RED))
        {
            //まだドンカチをもっていない？
            if (doseHaveHammer == false)
            {
